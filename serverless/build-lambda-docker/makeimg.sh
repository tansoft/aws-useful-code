...
echo -e "for local debug:\n    docker run -d -p 9000:8080 -e AWS_LAMBDA_FUNCTION_TIMEOUT=900 -e AWS_LAMBDA_FUNCTION_MEMORY_SIZE=1024 -e AWS_ACCESS_KEY_ID=xxx -e AWS_SECRET_ACCESS_KEY=xxx --name test-${project} ${project}:latest"
echo -e "  for test:\n    curl -XPOST "http://localhost:9000/2015-03-31/functions/function/invocations" -d '{}'"
echo -e "  for kill:\n    docker rm -f -v test-${project}"
echo -e "  for debug:\n    docker exec -it -t test-${project} /bin/bash\n    docker logs -t test-${project}"