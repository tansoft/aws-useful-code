...
function test_function() {
    ...
    ./stress-test -table ${table} -t 7200 -batchWriteItem 1 -config package-size.yaml -region ${region} "$@"
}
...
test_function ${ctable} -times 400

# 多行模式，写入1次是5份记录，写10000次，等于5万条记录
test_function ${rtable} -sortkey -times 10000
